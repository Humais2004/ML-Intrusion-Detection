...
int main(int argc, char** argv) {
    ...
    if (XGBoosterLoadModel(booster, "D:/ML-Intrusion-Detection/DesktopApp/src/xgboost_ids_model2.json") != 0) {
        if (rank == 0) std::cerr << "CRITICAL: Failed to load real_model.json.\n";
    ...
    // ---------------------------------------------------------
    if (rank == 0) {
        std::ifstream file("D:/ML-Intrusion-Detection/output2/downloads/binary_files/X_full.bin", std::ios::binary | std::ios::ate);
        if (!file.is_open()) {
